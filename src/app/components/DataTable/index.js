        ...
        this.config = {
            button: {
                ...
                extra:
                    props.config &&
                    props.config.button &&
                    props.config.button.extra
                        ? props.config.button.extra
                        : false,
            },
            filename:
                props.config && props.config.filename
                    ? props.config.filename
                    : 'table',
            key_column:
                props.config && props.config.key_column
                    ? props.config.key_column
                    : 'id',
            language: {
                length_menu:
                    props.config &&
            ...
            show_length_menu:
                props.config.show_length_menu !== undefined
                    ...
                    : true,
            show_filter:
                props.config.show_filter !== undefined
            ...
            show_pagination:
                props.config.show_pagination !== undefined
                    ...
                    : true,
            show_info:
                props.config.show_info !== undefined
                    ? props.config.show_info
                    : true,
            show_first:
                props.config.show_first !== undefined
                    ? props.config.show_first
                    : true,
            show_last:
                props.config.show_last !== undefined
...
//        let pageNumber = e.target.value
        ...
        for (let i in filterRecords) {
            ...
            for (let column of this.props.columns) {
                if (column.cell && typeof column.cell === 'function') {
                    let cellData = ReactDOMServer.renderToStaticMarkup(
                        column.cell(record, i)
                    )
    ...
    render() {
        ...
        if (this.props.dynamic === false) {
            ...
            totalRecords = Array.isArray(filterRecords)
                ? filterRecords.length
                : 0
            pages = this.pages = this.numPages(totalRecords)
            isFirst = this.isFirst()
            isLast = this.isLast()
            filterRecords = Array.isArray(filterRecords)
                ? this.paginate(filterRecords)
                : []
        } else {
                            ...
                            >
                                <tr>
                                    {this.props.columns.map((column, index) => {
                                        ...
                                        if (
                                            column.sortable &&
                                            this.state.sort.column === column.key
                                        ) {
